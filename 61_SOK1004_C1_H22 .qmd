...
```{r}
df <- lenke %>% 
  ApiData(Tid = paste(1970:2021), ContentsCode = c("BNP"))
```
...
as_tibble(df) #safe data as tibble

#kommentar
#sp??rsm??l: hvorfor blir df ikke tibble?


bnptib <- df  #variable name bnptib

head(df[[1]]) #first table

head(df[[2]]) #second table
